...
public:
	...
	void draw()
	{
		...
		this->add_entry("自瞄", &aimbot_tap, true);
		if (aimbot_tap) {
			this->add_entry("开启自瞄", &MenuConf.AimBootEx);
			this->add_entry("开启自瞄范围", &MenuConf.EnableFov);
			this->add_entry("自瞄范围", &MenuConf.FovRadios, 100.f, 300.f, 50.f);
		}
		...
		this->add_entry("其他功能", &misc_tab, true);
		if (misc_tab) {
		...
		this->add_entry("菜单", &menu_tab, true);
		if (menu_tab) {
			this->add_entry("  菜单x轴", &menux, 0.f, 2560.f, 50.f);
			this->add_entry("  菜单y轴", &menuy, 0.f, 1440.f, 50.f);
